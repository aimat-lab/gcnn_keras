...
from kgcnn.literature.inop import getmodelINOP
...
model = getmodelINOP(input_nodedim = 1,
                     input_edgedim =1,
                     input_envdim = 1,
                     Depth = 4,
                     node_dim = [64,32],
...
plt.plot(np.array(range(epostep,epo+epostep,epostep)),testlossall,label='Test Loss')
plt.xlabel('Epochs')
