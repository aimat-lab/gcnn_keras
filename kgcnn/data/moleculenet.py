...
class MuleculeNetDataset(DownloadDataset, MemoryGeometricGraphDataset):
    ...
    making graph-like features from smiles. The graph structure matches the molecular graph. The atomic coordinates
    are generated by a conformer guess. Since this require some computation time, it is only done once and and the
    molecular coordinate or mol-files stored in a json-file with name :obj:`MuleculeNetDataset.mol_filename`.
