...
class MoleculeNetDataset(MemoryGeometricGraphDataset):
    r"""Base class for using DeepChem molecule datasets style.

    The base class provides properties and methods for making graph features from smiles.
    The typical input is a `csv` or `excel` file with smiles and labels.

    The graph structure matches the molecular graph. The atomic coordinates
    are generated by a conformer guess. Since this require some computation time, it is only done once and the
    molecular coordinate or mol-blocks stored in a sdf-file with the base-name of :obj:``file_name``.
