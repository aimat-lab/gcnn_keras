"""
Layers to convert batched graphs into a single disjoint graph and vice verca.
The single graphs are defined by node_id and edge_id.
...
class CastRaggedToList(ks.layers.Layer):
    """ 
    Cast a batched ragged tensor like node or edge lists to a single list plus row length. 
    The shape follows something like (batch,None,F) --> ("batch*None",F).
    ...
    Output:
        ...
        The output shape is ("batch*None",F).
    """
...
class CastMaskedToList(ks.layers.Layer):
    """ 
    ...
    """
    def __init__(self, **kwargs):
        super(CastMaskedToList, self).__init__(**kwargs)
    def build(self, input_shape):
        super(CastMaskedToList, self).build(input_shape)
    def call(self, inputs):
        tens,mask = inputs
...
class CastBatchToList(ks.layers.Layer):
    """
    ...
    """
    def __init__(self, **kwargs):
        super(CastBatchToList, self).__init__(**kwargs)          
    def build(self, input_shape):
        super(CastBatchToList, self).build(input_shape)          
    def call(self, inputs):
...
class CastListToBatch(ks.layers.Layer):
    """
    Layer to add batchdim according to a reference of correct batch length. 
    ...
    """
    def __init__(self, **kwargs):
        super(CastListToBatch, self).__init__(**kwargs)          
    def build(self, input_shape):
        super(CastListToBatch, self).build(input_shape)          
    def call(self, inputs):
        infeat,ref = inputs
...
class CastListToPadded(ks.layers.Layer):
    """
    ...
    """
    def __init__(self, **kwargs):
        super(CastListToPadded, self).__init__(**kwargs)          
    def build(self, input_shape):
        super(CastListToPadded, self).build(input_shape)          
    def call(self, inputs):
        infeat,mask = inputs
...
class CastListToRagged(ks.layers.Layer):
    """
    ...
    """
    def __init__(self, **kwargs):
        super(CastListToRagged, self).__init__(**kwargs)          
    def build(self, input_shape):
        super(CastListToRagged, self).build(input_shape)          
    def call(self, inputs):
        nod,n_len = inputs
...
class CorrectIndexListForSubGraph(ks.layers.Layer):
    """ 
    Shifts the index for flatten index-tensors to accomodate the batch dimension.
    For example: flatten: [[0,1,2],[0,1],[0,1]] -> [0,1,2,0,1,0,1]
    shift: [0,1,2,1,1,0,1] -> [0,1,2,3+0,3+1,5+0,5+1]
    This is equivalent to one big batch-graph with disconnected subgraphs so
    that tf.gather would gather the correct nodes for a 1D tensor.
    ...
    Output:
        A shifted edge_index_list to match the correct nodes in flatten nodelist
    """
    def __init__(self, **kwargs):
...
class RaggedToDisjoint(ks.layers.Layer):
    """ 
    ...
    """
    def __init__(self,**kwargs):
        super(RaggedToDisjoint, self).__init__(**kwargs)
        self.cast_list = CastRaggedToList()
        self.correct_index = CorrectIndexListForSubGraph()
    def build(self, input_shape):
        super(RaggedToDisjoint, self).build(input_shape)
    def call(self, inputs):
