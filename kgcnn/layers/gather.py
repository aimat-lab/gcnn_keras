...
@tf.keras.utils.register_keras_serializable(package='kgcnn', name='GatherNodesTuple')
class GatherNodesTuple(GraphBaseLayer):
    """Gather nodes with a multiple outgoing nodes. E.g. for angles. Returns list.
    ...
    def __init__(self, selection_index, axis=1, axis_indices=2, **kwargs):
        """Initialize layer."""
        super(GatherNodesTuple, self).__init__(**kwargs)
        self.axis = axis
        self.axis_indices = axis_indices
        if not isinstance(selection_index, (list, tuple)):
            raise ValueError("Indices for selection must be list or tuple for layer `GatherNodesTuple`.")
        self.selection_index = list(selection_index)

    def build(self, input_shape):
        """Build layer."""
        super(GatherNodesTuple, self).build(input_shape)
        if len(input_shape) != 2:
            print("WARNING: Number of inputs for layer", self.name, "is expected to be 2.")
        if [i for i in range(len(input_shape[0]))][self.axis] == 0:
            print("WARNING: Shape error for", self.name, ", gather from batch-dimension is not intended.")
    ...
    def get_config(self):
        """Update config."""
        config = super(GatherNodesTuple, self).get_config()
        config.update({"axis": self.axis, "axis_indices": self.axis_indices, "selection_index": self.selection_index})
        return config
