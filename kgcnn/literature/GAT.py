...
import kgcnn.layers.disjoint.attention
...
def make_gat_disjoint(  # Input
    ...
    for i in range(0, depth):
        heads = [kgcnn.layers.disjoint.attention.AttentionHeadGAT(**attention_args)([nk, node_len, ed, edge_len, edi]) for _ in range(attention_heads_num)]
        if attention_heads_concat:
