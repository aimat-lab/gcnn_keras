...
from kgcnn.layers.keras import Dense, Concatenate, LayerNormalization
...
def make_graph_sage_disjoint(  # Input
        input_node_shape,
        input_edge_shape,
        input_embedd: dict = None,
        # Output
    ...
    for i in range(0, depth):
        ...
        eu = kgcnn.layers.disjoint.mlp.MLP(**edge_mlp_args)(eu)
        # Pool message
        if pooling_args['pooling_method'] in ["LSTM", "lstm"]:
            nu = kgcnn.layers.disjoint.pooling.PoolingLocalEdgesLSTM(**pooling_args)([n, node_len, eu, edge_len, edi])
        else:
            nu = kgcnn.layers.disjoint.pooling.PoolingLocalMessages(**pooling_args)([n, node_len, eu, edge_len, edi])  # Summing for each node connection

        nu = ks.layers.Concatenate()([n, nu])  # Concatenate node features with new edge updates

        n = kgcnn.layers.disjoint.mlp.MLP(**node_mlp_args)(nu)
        n = tf.keras.layers.LayerNormalization(axis=-1)(n)  # Normalize

    # Regression layer on output
    if output_embedd["output_mode"] == 'graph':
        out = kgcnn.layers.disjoint.pooling.PoolingNodes()([n, node_len])
