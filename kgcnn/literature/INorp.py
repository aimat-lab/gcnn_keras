...
import kgcnn.layers.ragged.gather
...
import kgcnn.layers.ragged.set2set
...
def make_inorp(  # Input
    ...
    for i in range(0, depth):
        ...
        # Add environment
        nu = Concatenate(axis=-1, input_tensor_type=tens_type)([n, nu, ev])  # Concatenate node features with new edge updates
...
def make_inorp_disjoint(  # Input
    ...
    ev = kgcnn.layers.ragged.gather.GatherState()([uenv, n])
    # n-Layer Step
    for i in range(0, depth):
        # upd = GatherNodes()([n,edi])
        eu1 = kgcnn.layers.ragged.gather.GatherNodesIngoing()([n, edi])
        eu2 = kgcnn.layers.ragged.gather.GatherNodesOutgoing()([n, edi])
        upd = ks.layers.Concatenate(axis=-1)([eu2, eu1])
        eu = ks.layers.Concatenate(axis=-1)([upd, ed])

        eu = kgcnn.layers.ragged.mlp.MLPRagged(**edge_mlp_args)(eu)
        # Pool message
        nu =  kgcnn.layers.ragged.pooling.PoolingLocalEdges(**pooling_args)([n, eu, edi])  # Summing for each node connection
        # Add environment
        nu = ks.layers.Concatenate()([n, nu, ev])  # Concatenate node features with new edge updates

        n = kgcnn.layers.ragged.mlp.MLPRagged(**node_mlp_args)(nu)

    if output_embedd["output_mode"] == 'graph':
        if use_set2set:
            # output
            outss = kgcnn.layers.ragged.conv.DenseRagged(set2set_args["channels"])(n)
            out = kgcnn.layers.ragged.set2set.Set2Set(**set2set_args)(outss)
        else:
            out =  kgcnn.layers.ragged.pooling.PoolingNodes()(n)

        main_output = kgcnn.layers.disjoint.mlp.MLP(**output_mlp)(out)

    else:  # Node labeling
        out = n
        main_output = kgcnn.layers.ragged.mlp.MLPRagged(**output_mlp)(out)
