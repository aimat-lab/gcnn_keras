...
from kgcnn.layers.keras import Concatenate, Dense, Add, Dropout
...
def make_megnet_disjoint(
    ...
    up = uenv
    ep = ed
    vp = kgcnn.layers.disjoint.mlp.MLP(**node_ff_args)(vp)
    ep = kgcnn.layers.disjoint.mlp.MLP(**edge_ff_args)(ep)
    up = kgcnn.layers.disjoint.mlp.MLP(**state_ff_args)(up)
    ep2 = ep
    vp2 = vp
    up2 = up
    for i in range(0, nblocks):
        if has_ff and i > 0:
            vp2 = kgcnn.layers.disjoint.mlp.MLP(**node_ff_args)(vp)
            ep2 = kgcnn.layers.disjoint.mlp.MLP(**edge_ff_args)(ep)
            up2 = kgcnn.layers.disjoint.mlp.MLP(**state_ff_args)(up)

        # MEGnetBlock
        vp2, ep2, up2 = kgcnn.layers.disjoint.conv.MEGnetBlock(**meg_block_args)([vp2, ep2, edi, up2, node_len, edge_len])
    ...
    if use_set2set:
        ...
        vp = kgcnn.layers.disjoint.set2set.Set2Set(**set2set_args)([vp, node_len])
        ep = kgcnn.layers.disjoint.set2set.Set2Set(**set2set_args)([ep, edge_len])
    else:
        vp = kgcnn.layers.disjoint.pooling.PoolingNodes()([vp, node_len])
        ep = kgcnn.layers.disjoint.pooling.PoolingGlobalEdges()([ep, edge_len])
    ...
    # final dense layers 
    main_output = kgcnn.layers.disjoint.mlp.MLP(**output_mlp)(final_vec)
