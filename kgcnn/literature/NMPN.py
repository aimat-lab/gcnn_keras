...
def getmodelNMPN(
    ...
    Args:
        ...
        output_dim (list): Output dimension. Optionally list for multiple layer.
                           Default is [25,10,1].
        output_activation (list): Activation function. Optionally list for multiple layer.
                                  Default is ['selu','selu','sigmoid'].
    ...
    edge_net = ks.layers.Dense(node_dim * node_dim, activation=activation)(ed)
    gru = GRUupdate(node_dim)

    for i in range(0, depth):
        eu = GatherNodesOutgoing()([n, node_len, edi, edge_len])
        eu = ApplyMessage(node_dim)([edge_net, eu])
        eu = PoolingEdgesPerNode(is_sorted=is_sorted, has_unconnected=has_unconnected)(
    ...
    if output_embedd == 'graph':
        ...
        for j in range(len(output_dim) - 1):
            out = DenseRagged(output_dim[j], activation=output_activation[j], use_bias=output_use_bias[j],
                              bias_regularizer=output_bias_regularizer[j],
                              activity_regularizer=output_activity_regularizer[j],
                              kernel_regularizer=output_kernel_regularizer[j])(out)
        main_output = DenseRagged(output_dim[-1], name='main_output', activation=output_activation[-1],
                                  use_bias=output_use_bias[-1],
