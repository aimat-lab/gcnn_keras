...
def make_schnet_disjoint(
    ...
    for i in range(0, depth):
        n = kgcnn.layers.disjoint.conv.SchNetInteraction(**interaction_args)([n, node_len, ed, edge_len, edi])

    n = kgcnn.layers.disjoint.mlp.MLP(**output_mlp)(n)
    ...
    if output_embedd["output_mode"] == 'graph':
        if out_scale_pos == 0:
            n = mlp_last(n)
        out = kgcnn.layers.disjoint.pooling.PoolingNodes(**node_pooling_args)([n, node_len])
        if out_scale_pos == 1:
            out = mlp_last(out)
        main_output = ks.layers.Flatten()(out)  # will be dense
    else:  # node embedding
        ...
        main_output = kgcnn.layers.ragged.casting.CastRaggedToDense()(main_output)  # no ragged for distribution atm
