...
from kgcnn.layers.ragged.gather import GatherState,GatherNodesIngoing,GatherNodesOutgoing,LazyConcatenateNodes
from kgcnn.layers.ragged.denseconv import DenseRagged
from kgcnn.layers.ragged.pooling import PoolingEdgesPerNode,PoolingNodes
from kgcnn.layers.ragged.set2set import Set2Set


def getmodelINOP(input_nodedim,
            input_edgedim,
            input_envdim,
            Depth = 1,
    ...
    for i in range(0,Depth):
        ...
        nu = PoolingEdgesPerNode(pool_method=pooling_method )([n,eu,edi]) # Summing for each node connection
        
        nu = LazyConcatenateNodes()([n,nu])
        if(add_env == True):
