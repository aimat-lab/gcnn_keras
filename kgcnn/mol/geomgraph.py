...
class GeometricMolGraph:
    ...
    def __init__(self, atom_labels=None, coordinates=None, atom_number=None):
        ...
        if self.atom_labels is not None and self.atom_number is None:
            self.atom_number = np.array([self.global_proton_dict[x] for x in self.atom_labels], dtype="int")

        if self.atom_number is not None and self.atom_labels is None:
            self.atom_labels = [self.inverse_global_proton_dict[int(x)] for x in self.atom_number]
