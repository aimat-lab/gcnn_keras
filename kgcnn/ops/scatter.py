...
def _tensor_scatter_nd_mean(tensor, indices, updates):
    ...
    counts_tensor = tf.zeros(
        tf.concat([tf.shape(tensor)[:tf.shape(indices)[-1]], tf.ones_like(tf.shape(tensor))[tf.shape(indices)[-1]:]],
                  axis=0), dtype=tensor.dtype)
    counts_updates = tf.ones(
        tf.concat([tf.shape(updates)[:tf.shape(indices)[-1]], tf.ones_like(tf.shape(updates))[tf.shape(indices)[-1]:]],
                  axis=0), dtype=updates.dtype)
